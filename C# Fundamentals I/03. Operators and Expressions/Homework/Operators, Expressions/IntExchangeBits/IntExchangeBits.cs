...
namespace IntExchangeSpecificBits
{
    ...
    class IntExchangeBits
    {
        static void Main()
        {
            ...
            Console.WriteLine("Unsigned integer binary value = {0}", Convert.ToString(uintNumber, 2).PadLeft(32, '0'));
            ...
            Console.WriteLine("Bits p position: ({0}, {1}+{2}-1)", p, p, k);

            foreach (uint myBitsP in bitsP)
            {
                Console.WriteLine(myBitsP);
            }
            ...
            Console.WriteLine("Bits q position: ({0}, {1}+{2}-1)", q, q, k);

            foreach (uint myBitsQ in bitsQ)
            {
                Console.WriteLine(myBitsQ);
            }
            ...
            Console.WriteLine("Result integer binary value = {0}", Convert.ToString(resultReplace, 2).PadLeft(32, '0'));
            Console.ReadLine();
        }
    }
}
