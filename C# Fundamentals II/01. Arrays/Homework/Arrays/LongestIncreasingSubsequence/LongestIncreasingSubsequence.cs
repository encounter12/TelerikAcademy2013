...
{
    class ArrayLongestIncreasingSubsequence
    {
        /*Write a program that reads an array of integers and removes from it a minimal number of elements in such way that the remaining array is 
         * sorted in increasing order. Print the remaining sorted array. Example: {6, 1, 4, 3, 0, 3, 6, 4, 5}  {1, 3, 3, 4, 5}
         */
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("The longest increasing subsequence in {" + string.Join(", ", array.Select(x => x.ToString()).ToArray()) + "} is:");
            Console.WriteLine("{" + string.Join(", ", longestIncreasingSubsequence.Select(x => x.ToString()).ToArray()) + "}");
        }
